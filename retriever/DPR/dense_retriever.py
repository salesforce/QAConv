...
def save_results(
    ...
    for i, q in enumerate(questions):
        ...
        try:
            ...
            print("results_and_scores[0]", results_and_scores[0])
            prinit("passages.keys()", passages.keys())
            exit(1)
...
def main(cfg: DictConfig):
    ...
    data = []
    for ctx_file in cfg.encoded_ctx_files:
        d = pickle.load(open(ctx_file, "rb"))
        data += d
    data = np.array([d[1] for d in data])
    print(np.shape(data))
    scores = np.dot(data, np.transpose(questions_tensor.numpy()))
    ...
    for i in tqdm(range(len(questions))):
        ...
        score = score[rank]
        top_ids_and_scores.append([ ["qaconv:"+str(pos) for pos in rank], score])
