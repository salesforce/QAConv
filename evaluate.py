...
def compute_fuzz_ratio(a_gold, a_pred):
  return fuzz.ratio(normalize_answer(a_pred), normalize_answer(a_gold))

def add_word_number_mapping(answers):
    added_ans = []
    for ans in answers:
        if ans.isdigit():
...
def compute_unanswerable_binary(a_gold_list, a_pred_list):
...
if __name__ == '__main__':
  ...
  if OPTS.folder:
      ...
      for f in glob.glob(f"{OPTS.pred_file}/*.json"):
          ...
          OPTS.pred_file = f
          dic =  main(OPTS)
          dic.update({"Name":f.replace(f"{OPTS.pred_file}/","")})
          table.append(dic)
